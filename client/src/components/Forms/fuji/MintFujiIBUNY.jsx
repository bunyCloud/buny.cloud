...
function MintFujiIBUNY() {
  const {chainId, account, accountAddress} = useContext(AppContext)
  ...
  const nftAddress = FujiTheBUNY.address
  ...
  const fetchIBUNYData = useCallback(async () => {
    ...
    const nftContract = new ethers.Contract(FujiTheBUNY.address, FujiTheBUNY.abi, provider)
  ...
  const MintFujiIBUNY = async () => {
    try {
    const provider = new ethers.providers.Web3Provider(window.ethereum)
    const signer = provider.getSigner()
    const contract = new ethers.Contract(FujiTheBUNY.address, FujiTheBUNY.abi, signer)
    ...
    setRefreshTokenList(prevState => !prevState); // Toggle the state
    toast({
  ...
  return (
    <VStack bg='white' color='black' spacing={1} w={'100%'} p={1} mt={1}>
                  ...
                  <VStack w="100%">
                    ...
                    <div>
                      {mintResult && (
                        <>
                          <Box mb={2}>
                            <AntButton href={`https://testnet.snowtrace.io/tx/${mintResult}`} target="_blank" >
                              View Transaction
                            </AntButton>
                            </Box>
      ...
      <Button size={'sm'}  colorScheme='twitter'  onClick={() => MintFujiIBUNY('1')}>
      ...
      <FetchMyIBUNY contractAddress={FujiTheBUNY.address} abi={FujiTheBUNY.abi} refresh={refreshTokenList} />
      
    </VStack>
  )
}

export default MintFujiIBUNY
